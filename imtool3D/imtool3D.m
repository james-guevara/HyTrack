classdef imtool3D < handle
    ...
    methods
        ...
        function tool = imtool3D(varargin)  %Constructor
            ...
            tool.handles.Tools.PaintBrush        = uicontrol(tool.handles.Panels.ROItools,'Style','togglebutton','String','','Position',[buff buff+4*w w w],'TooltipString','PaintBrushTool');
            icon_profile = makeToolbarIconFromPNG([MATLABdir '/tool_data_brush.png']);
            set(tool.handles.Tools.PaintBrush ,'Cdata',icon_profile)
            fun=@(hObject,evnt) PaintBrushCallback(hObject,evnt,tool);
        ...
        function createBrushObject(tool)
...
function PaintBrushCallback(hObject,evnt,tool)

if get(hObject,'Value')
    createBrushObject(tool);
else
